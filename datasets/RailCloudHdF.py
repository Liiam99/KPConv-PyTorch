...
from os.path import exists, join, isdir, isfile, splitext
...
class RailCloudHdFDataset(PointCloudDataset):
    ...
    def prepare_RailCloudHdF_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            las = laspy.read(join(self.path, self.set, f'{cloud_name}.laz'))

            # Initiate containers
            cloud_x = np.array(las.x, dtype=np.float32)
            cloud_y = np.array(las.y, dtype=np.float32)
            cloud_z = np.array(las.z, dtype=np.float32)
            ...
            # Save as ply
            field_names = ['x', 'y', 'z', 'intensity', 'class']
            write_ply(join(ply_path, cloud_name + '.ply'), [cloud_points, intensity, labels], field_names)
